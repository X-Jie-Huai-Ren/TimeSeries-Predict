"""
Train rnn model for Synthetic data
...
def main(arg_dict):
    ...
    model = imported_model.Model(input_dim=config.FEATURE_DIM, output_dim=1).to(config.DEVICE)
    # Optimizer
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # train for epoch
        loss = train_for_rnn(train_loader, model, optimizer, criterion, epoch, device=config.DEVICE)
